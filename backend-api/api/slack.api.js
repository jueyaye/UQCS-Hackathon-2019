...
function sendTemplate(project, blocks) {
  try {
    ...
    logger.error("SLACK API SEND MESSAGE:", "error:", err);
    throw err;
  }
}
        ...
        if (body.payload) {
          ...
          if (payload.type === "message_action") {
            ...
            for (let i = 0; i < cards.length; i += 1)
              if (cards[i].name === "step1") card = cards[i];
            ...
            if (message.files) {
              ...
              return await trello.addAttachmentToCard(
                project,
                card,
                publicFile.permalink_public
              );
            }
            ...
            if (message.blocks) {
              ...
              return await trello.addCommentToCard(
                project,
                card,
                text.text.text
              );
          ...
          if (action.type === "static_select") {
            ...
            if (selected.element === "card") {
              const card = await trello.getCard(
                project,
                project.trelloDetails,
                { id: selected.id }
              );
              ...
              return await sendTemplate(
                project,
                messageTemplate(`${card.shortUrl}`)
              );
            }
            if (selected.element === "commit") {
              const commit = await github.fetchCommit(project, {
                sha: selected.id
              });
              ...
              return await sendTemplate(
                project,
                messageTemplate(`${commit.url}`)
              );
            }
            if (selected.element === "pull-request") {
              const pr = await github.fetchPullRequest(project, {
                number: selected.id
              });
    ...
    async (req, res) => {
      try {
        ...
        if (body.command === "/link") {
          ...
          if (!project)
            throw new Error("No project id found matching supplied id.");
        ...
        if (body.command === "/tag") {
          ...
          if (element === "card") {
            ...
            return await sendTemplate(
              project,
              searchTemplate("cards", cardsParsed)
            );
          }
          ...
          if (element === "commit") {
            ...
            return await sendTemplate(
              project,
              searchTemplate("commits", commitsParsed)
            );
          }
          ...
          if (element === "pull-request") {
            ...
            return await sendTemplate(
              project,
              searchTemplate("pull requests", pullsParsed)
            );
          }
