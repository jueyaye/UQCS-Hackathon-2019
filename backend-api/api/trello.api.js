...
function search(project, query){
  const trello = new Trello(project.trelloCredentials.key, project.trelloCredentials.token);
...
function addWebhook(project, board){
  const trello = new Trello(project.trelloCredentials.key, project.trelloCredentials.token);
  return new Promise((resolve, reject) => {
    trello.addWebhook(project.id, 
      `${config.trello.callback.url}/webhook/trello/events/${project.id}`, board.id, (error, data) => {
      if(error) reject(error);
...
function archiveList(project, list){
  const trello = new Trello(project.trelloCredentials.key, project.trelloCredentials.token);
  return new Promise((resolve, reject) => {
    trello.makeRequest("PUT", `/1/lists/${list.id}/closed`, { value: true }, (error, data) => {
      if(error) reject(error);
    ...
    if(body.action.type === "updateCard"){
      ...
      if(list.name === 'Doing'){
        ...
        for(var i = 0; i < branches.length; i += 1){
          if(branches[i].ref === "refs/heads/master")
            master = branches[i];
        }

        await github.createBranch(project, 
          `refs/heads/${body.action.data.card.name}`, master.object.sha);
      }

      if(list.name === 'Code review'){
        logger.debug(body.action.data.card.name);

        await github.createPullRequest(project, `Merge ${body.action.data.card.name}`, 
        `refs/heads/${body.action.data.card.name}`, "refs/heads/master")
      }
    }
